...
function lineSearch(object_function::Function, diff_function::Function, hessian_function::Function, x_vec::Vector, max_iter_num::Int; BK_method="BFGS")
    """
    Line Search Method without contraints: 

    Paramerter ----
    object_function: the object function, i.e. f(x)
    diff_function: the differential function, i.e. f'(x)
    hessian_function: the real hessian matrix, i.e. δ^2f(x)
    ...
    for i in range(1, length=max_iter_num)
        ...
        # Calculate B_k
        if (BK_method=="BFGS")
            B_k = BFGS(B_k, x_vec_current, x_vec_previous, diff_function) # BFGS
        elseif (BK_method=="Newtown")
            B_k = hessian_function(x_vec_current) # the Newtown method
        elseif (BK_method=="Speedest_Descend")
            B_k = steepestDescentBkMatrix(x_vec_current) # the steepest descent method
        end

        # Calculate the descent direction
        p_vec = -inv(B_k)*diff_function(x_vec_current)
        # Calculate the step length
        alpha = backtrackingLineSearch(object_function, diff_function, x_vec_current, p_vec)
...
function BFGS(B_k_pre, x_vec::Vector, x_vec_pre::Vector, diff_function::Function)
    s_k = x_vec - x_vec_pre
    y_k = diff_function(x_vec) - diff_function(x_vec_pre)
    tolerance = 1e-16
